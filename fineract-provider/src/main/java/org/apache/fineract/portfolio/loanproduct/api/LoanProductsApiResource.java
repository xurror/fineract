...
public class LoanProductsApiResource {
    ...
    public LoanProductsApiResource(final PlatformSecurityContext context, final LoanProductReadPlatformService readPlatformService,
            ...
            final ConfigurationDomainService configurationDomainService, final CreditScorecardFeatureRepositoryWrapper featureRepository) {
    ...
    private LoanProductData handleTemplate(final LoanProductData productData) {
        ...
        final Collection<CreditScorecardFeatureData> scorecardFeatureOptions = this.featureRepository.findAllWithNotFoundDetection()
                .stream().map(CreditScorecardFeature::toData).collect(Collectors.toList());
